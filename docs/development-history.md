# Development reference and historical milestones
...
The desktop now tracks GPU, CPU, and display profiles independently. Refreshing GPU readings leaves active CPU/display settings alone. Recovery retries are bounded, GPU restoration remains available after CPU failure, and startup closes pending GPU journals even after a driver reset. A normal-user CPU companion attempts exact restoration if the desktop dies. AMD control requires a successfully queried driver default and remains experimental.

See [the current architecture and recovery behaviour](current-architecture.md). Build and run all non-mutating checks with `./packaging/Test-Safety.ps1`; build and verify an installer with `./packaging/Build-Release.ps1`.

## Historical milestones

The following records the development sequence; statements about earlier UI restrictions are historical:
...
Only after following [the supervised operator procedure](live-gpu-canary.md), run the fixed live canary:
...
Only after following [the live recovery drill procedure](live-recovery-drill.md), run:
...
Only after following [the manual persistent live toggle procedure](manual-persistent-live-toggle.md), run one fixed toggle operation:
...
Only after following [the CPU maximum processor state procedure](cpu-maximum-processor-state.md), run the five-second 80% canary:
...
Follow [the screen overlay canary procedure](screen-overlay-canary.md). The test uses `Ctrl+Shift+F12` as a global emergency exit and also dismisses every overlay automatically after 15 seconds.

The integrated main-app screen dimming controls are described in [the desktop screen-dimming procedure](desktop-screen-dimming.md). They are optional and use the primary activation button, assigned hotkey, or enabled idle timer; no second hotkey or UAC prompt is introduced.
...
## Historical 1.24 release candidate
...
For public Authenticode releases, follow [the code-signing procedure](code-signing.md). The release pipeline can require a certificate-store identity, sign and timestamp every shipped executable and library before packaging, sign the outer installer, and verify every resulting signature.
